...
Just like firebase-realtime/migration.js, this script relies on `runQueries`, but with two key differences:
	1. It doesn't query any data via API
	2. It requires the two Firebase export json files

## 1. Export Firebase database and auth

```
npm install --global firebase-tools
firebase login
firebase database:get ./input/migration-script/firebase-db-export.json --project firebase-radio4000 /
firebase auth:export ./input/migration-script/firebase-auth-export.json --project firebase-radio4000
```

Also see https://firebase.google.com/docs/cli/auth#auth-export.

## 2. Prepare fresh Postgres database
...
Check the source code for how it works. It's faster with bun, but should work in node as well.
 ...
 */
import firebaseExport from './firebase-database-export.json' assert {type: 'json'}

/** @type {{users: Array<FirebaseAuthUser>}} */
import firebaseAuthExport from './firebase-auth-export.json' assert {type: 'json'}
...
async function createSupabaseUser(backup) {
	...
	const res = await supabaseAdminClient.auth.signUp({
		...
		// options: {
		// 	data: {
		// 		// we can put stuff here, and access on auth.users.raw_user_metadata?
		// }
	})
	if (res.error) {
...
function getAuthUser(uid) {
	return firebaseAuthExport.users.find((u) => u.localId === uid)
}
...
function serializeChannel(channel) {
	...
	// Set "imageUrl"
	if (channel.image) channel.imageUrl = `https://res.cloudinary.com/radio4000/image/upload/${channel.image}`
