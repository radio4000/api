...
async function getChannelBackup(channelSlug) {
	const channelBackup = await getChannelBySlug(channelSlug)
	if (channelBackup) {
		channelBackup.tracks = await getChannelTracks(channelBackup.id)
	}
...
function serializeChannel(channel) {
	...
	channel.r4FirebaseId = channel.id
...
export {getAuth, getUserExport, getChannelBySlug, getChannelBackup}
