...
const {YOUTUBE_API_KEY: key} = config
...
export default async function handler(req, res) {
	...
	try {
		...
		if (!key) throw new Error('A YOUTUBE_KEY in your .env file is required')
		...
		const data = await fetch(
			`https://www.googleapis.com/youtube/v3/videos?part=status,contentDetails,snippet&id=${ytid}&key=${key}`
		)
		const json = await data.json()

		// It does not throw with no results, so we check here.
		if (json.items.length === 0) {
			return res.status(404).json({message: 'YouToube video not found'})
		}

		// Serialize and return.
		res.status(200).json(serialize(json.items[0]))
	} catch (err) {
